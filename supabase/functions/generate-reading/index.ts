  ...
  try {
    ...
    const prompts: Record<string, string> = {
      ...
      stars_birth_chart: [
        ...
        `🌙 MOON SIGN — Emotional world, instincts, and inner needs. 2-3 sentences. ${birthTime && birthTime !== "Unknown" ? `The birth time is ${birthTime} — interpret the likely Moon sign energy and emotional archetype it suggests.` : `Birth time was not provided, so describe the emotional and intuitive archetype associated with ${sign} rather than naming a specific Moon sign.`}`,
        `⬆️ RISING SIGN (ASCENDANT) — Outward personality and first impressions. 2-3 sentences. ${birthTime && birthTime !== "Unknown" && birthPlace && birthPlace !== "Unknown" ? `The birth time is ${birthTime} and place is ${birthPlace} — describe the rising sign energy and outward personality it suggests.` : `Birth time or place was not provided. Note this briefly and instead describe the outward personality qualities most common for ${sign} sun signs.`}`,
